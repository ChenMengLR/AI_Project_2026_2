...
- API 状态：`week03_rule_chatbot/.env` 与 `week02_photo_classifier/.env` 都没有有效的 `DASHSCOPE_API_KEY` / `DASHSCOPE_BASE_URL`；`--test-api` 因配置不完整安全退出，未发起请求。
...
- 工作树：本次修改仅涉及 `week03_rule_chatbot/rules.txt`、`tests/test_week03_chatbot.py` 和本状态文件；另有未跟踪 `.githooks/` 目录未纳入本次提交。
